...
/***/ function(module, exports) {
	...
	var path = 'http://localhost/msfalcons/www/';
	...
	d3.select(".config__language").on("change", function (data) {
	    // clear_row_left(null);
	    // clear_row_right(null);
	    d3.select(".solution").property("value", "");
	    init_rows(d3.select(".solution").property("value"));
	});

	d3.select(".solution").on("input", debounce(function (data) {
	    clear_row_left(null);
	    clear_row_right(null);

	    var validTeluguRE = /[^\u0C00-\u0C7F]/gi;
	    var validEnglishRE = /[^a-z]/gi;

	    var language = d3.select(".config__language").property("value");
	    var hasSpecialChars = false;

	    if (language === "telugu") {
	        //accept only telugu charaters
	        init_rows(this.value.replace(validTeluguRE, ""));
	        hasSpecialChars = validTeluguRE.test(this.value);
	    } else if (language === "english") {
	        //accept only english characters
	        init_rows(this.value.replace(validEnglishRE, ""));
	        hasSpecialChars = validEnglishRE.test(this.value);
	    }

	    if (hasSpecialChars) {
	        d3.select('#validate-message').classed("hide", false);
	    } else {
	        d3.select('#validate-message').classed("hide", true);
	    }
	}, 1000));
	...
	function load_left(row, val) {
	    ...
	    var url = path + 'api.php?type=pair&word=' + encodeURI(val);
	    //populate clues
	...
	function init_left(row) {
	    d3.select("#row-" + row + " .side--right .side__word").on("input", debounce(function () {
	    ...
	    d3.select("#row-" + row + " .side--left .side__word").on("input", function () {
	...
	d3.selectAll('input[type="file"]').on("change", function () {
	    var src = '';
	...
	d3.select('#config__image_radio--url ~ .input-field input').on("input", debounce(function () {
	    src = this.value;
	    d3.select('.image').property('src', src);
	}, 1000));

	d3.selectAll('input[name="config__image"]').on("change", function () {
	    if (this.value != 0) {
	        var _src = '';
	        if (this.value == 1) {
	            d3.select('#config__image_radio--upload ~ .input-field .btn').attr('disabled', 'disabled').attr('required', null);
	            d3.select('#config__image_radio--url ~ .input-field input').attr('disabled', null).attr('required', 'required');
	            _src = d3.select('#config__image_radio--url ~ .input-field input').property("value");
	        }
	        if (this.value == 2) {
	            d3.select('#config__image_radio--upload ~ .input-field .btn').attr('disabled', null).attr('required', 'required');
	            d3.select('#config__image_radio--url ~ .input-field input').attr('disabled', 'disabled').attr('required', null);
	            _src = window.URL.createObjectURL($('#config__image_radio--upload ~ .input-field input').get(0).files[0]);
	        }
	        d3.select('.image').property('src', _src);
	        d3.select('.image__wrapper').classed('hide', false);
	    } else {
	        d3.select('#config__image_radio--upload ~ .input-field .btn').attr('disabled', 'disabled').attr('required', null);
	        d3.select('#config__image_radio--url ~ .input-field input').attr('disabled', 'disabled').attr('required', null);
	        d3.select('.display__image').style("display", "none");
	...
	function update_row_left(row, data, val) {
	    ...
	    rowleft.exit().remove();
	    rowleft.enter().append("option").text(function (d) {
	        return d.value_name;
	    });
	}
	...
	function init_rows(word) {
	    ...
	    req[0] = d3.json(url, function (data) {
	        ...
	        data.forEach(function (data, i) {
	            ...
	            var char = encodeURI(data);
	            if (char === '') return;
	            var pos = slider_column_preference.noUiSlider.get() - 1;
	            var min = slider_number_of_characters.noUiSlider.get()[0];
	            var max = slider_number_of_characters.noUiSlider.get()[1];
	            var lang = d3.select(".config__language").property("value");
	            var url = path + '/api.php?type=word&char=' + char + '&pos=' + pos + '&lang=' + lang + '&min=' + min + '&max=' + max;
